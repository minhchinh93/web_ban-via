...
public function  categoriesDetail(Request $request, $id){
    $keyword = $request->keyword;
    $count= product::where('id_type',$id)->withTrashed()->count();
if($count==0){
    $shows=  product::where('id_type',$id)->withTrashed()->paginate(10)->products;
    return view('admin.products.index',['shows'=>$shows]);
}else{
    $shows=  product::where('id_type',$id)->withTrashed()->paginate(10);

    if($shows->total()>0){
    $count= product::where('id',$id)->withTrashed()->count();
    $trackuser= product::where('id_type',$id)->onlyTrashed()->count();
    $activeruser = product::where('id_type',$id)->where('deleted_at',null)->count();
    ...
    public function postCategory(Request $request){
        ...
        type_product::create(array_merge($request->only('name','description','price'),['image'=>$image]));
    ...
    public function updateCategory(Request $request, $id){
        ...
        type_product::where('id',$id)->update(array_merge($request->only('name','description','price'),['image'=>$image]));
